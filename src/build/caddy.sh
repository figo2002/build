...
for ARCH in ${ARCHS[@]}; do
	if [ "${ARCH}" == "arm" ]; then
		for ARM in ${ARMS[@]}; do
			...
			openssl dgst --sha256 release/caddy-linux-${ARCH}32-v${ARM} | sed 's/([^)]*)//g' >> release/caddy-linux-${ARCH}32-v${ARM}.hash
			openssl dgst --sha512 release/caddy-linux-${ARCH}32-v${ARM} | sed 's/([^)]*)//g' >> release/caddy-linux-${ARCH}32-v${ARM}.hash
		done
	else
		...
		openssl dgst --sha256 release/caddy-linux-${ARCH} | sed 's/([^)]*)//g' >> release/caddy-linux-${ARCH}.hash
		openssl dgst --sha512 release/caddy-linux-${ARCH} | sed 's/([^)]*)//g' >> release/caddy-linux-${ARCH}.hash
	fi
done