...
for (( i = 0; i <= 9; i++ )); do
	...
	openssl dgst --sha256 release/trojan-r-${RELEASE[i]} | sed 's/([^)]*)//g' >> release/trojan-r-${RELEASE[i]}.hash
	openssl dgst --sha512 release/trojan-r-${RELEASE[i]} | sed 's/([^)]*)//g' >> release/trojan-r-${RELEASE[i]}.hash
done
